		...
		Pass
		{
			CGPROGRAM
				...
				float4 frag(v2f_img IN) : COLOR
				{
					...
					color = UNITY_SAMPLE_TEX2D(_MainTex, uv) * 0.003663003;
					// (-1, -2) wt 4/273
					uv = saturate(IN.uv + float2(-1.0 * _ImageDims.z, -2.0 * _ImageDims.w));
					color += UNITY_SAMPLE_TEX2D(_MainTex, uv) * 0.014652014;
					// (+0, -2) wt 7/273
					uv = saturate(IN.uv + float2(+0.0 * _ImageDims.z, -2.0 * _ImageDims.w));
					color += UNITY_SAMPLE_TEX2D(_MainTex, uv) * 0.025641025;
					// (+1, -2) wt 4/273
					uv = saturate(IN.uv + float2(+1.0 * _ImageDims.z, -2.0 * _ImageDims.w));
					color += UNITY_SAMPLE_TEX2D(_MainTex, uv) * 0.014652014;
					// (+2, -2) wt 1/273
					...
					color += UNITY_SAMPLE_TEX2D(_MainTex, uv) * 0.058608058;
					// (+0, -1) wt 24/273
					uv = saturate(IN.uv + float2(+0.0 * _ImageDims.z, -1.0 * _ImageDims.w));
					color += UNITY_SAMPLE_TEX2D(_MainTex, uv) * 0.087912087;
					// (+1, -1) wt 16/273
					uv = saturate(IN.uv + float2(+1.0 * _ImageDims.z, -1.0 * _ImageDims.w));
					color += UNITY_SAMPLE_TEX2D(_MainTex, uv) * 0.058608058;
					// (+2, -1) wt 4/273
